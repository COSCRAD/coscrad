...
// TODO Allow the user to specify a custom data snapshot
// This file is generated when you run "validateTestData.spec.ts"
const testData = require(process.env
    .ARANGODB_DESTINATION_CONTAINER_DOCKER_SHARED_VOLUME_SCRIPTS_PATH +
    '/test-data/testData.data.json');

for (collectionType in testData) {
    for (collectionName in testData[collectionType]) {
        addCollectionAndData(
            collectionName,
            collectionType,
            testData[collectionType][collectionName]
        );
    }
}
...
function addCollectionAndData(collectionName, collectionType, data) {
    print(
        `Attempting to add ${collectionType} collection "${collectionName}" to ${process.env.ARANGO_DB_NAME}`
    );
    let success = false;
