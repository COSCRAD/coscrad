...
@CoscradDataExample<DigitalTextPagesViewModel>({
    example: {
...
export class DigitalTextPagesViewModel implements ApplyEvent<DigitalTextPagesViewModel> {
    ...
    constructor(dto: DTO<DigitalTextPagesViewModel>) {
        if (!dto) return;

        const { id, name, isPublished, tags, pages, accessControlList, notes, connections } = dto;
    ...
    private addPage<T extends DigitalTextPagesViewModel>(
        this: T,
    ...
    public static fromDto(dto: DTO<DigitalTextPagesViewModel>) {
        return new DigitalTextPagesViewModel(dto);
    }

    public forUser(userWithGroups: CoscradUserWithGroups): Maybe<DigitalTextPagesViewModel> {
        /**
    ...
    public static fromDigitalTextCreated(creationEvent: DigitalTextCreated) {
        const view = new DigitalTextPagesViewModel({
            id: creationEvent.payload.aggregateCompositeIdentifier.id,
        } as DTO<DigitalTextPagesViewModel>).apply(creationEvent);
