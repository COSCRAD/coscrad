...
const dummyFsa = buildTestCommandFsaMap().get(commandType) as CommandFSA<CreateMediaItem>;

const commandFsaFactory = new DummyCommandFsaFactory<CreateMediaItem>((id) =>
...
describe(`${commandType} (payload type validation)`, () => {
    describe(`when the command payload type is invalid`, () => {
        describe(`when the aggregate type is not mediaItem`, () => {
            Object.values(AggregateType)
                .filter((aggregateType) => aggregateType !== AggregateType.mediaItem)
                .forEach((invalidAggregateType) => {
                    ...
                    const commandInstance = plainToInstance(CreateMediaItem, invalidFsa.payload);
