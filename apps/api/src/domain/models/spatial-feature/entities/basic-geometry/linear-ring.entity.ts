 ...
 */
export class LinearRing extends BaseDomainModel implements IToPlainArray<number[][]> {
    readonly points: Position2D[];

    constructor({ points }: DTO<LinearRing>) {
        super();

        this.points = points.map((point) => new Position2D(point));
    }
