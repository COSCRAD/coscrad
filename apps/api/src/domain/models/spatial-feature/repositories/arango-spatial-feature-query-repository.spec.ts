...
describe(`ArangoSpatialFeatureRepository`, () => {
    ...
    const buildSpatialFeatureText = (id: string) => `spatial feature ${id}`;

    const spatialFeatureText = buildSpatialFeatureText(spatialFeatureIds[0]);

    const originalLanguageCode = LanguageCode.Chilcotin;

    const spatialFeatureViews: SpatialFeatureViewModel[] = spatialFeatureIds.map((id) =>
        buildTestInstance(SpatialFeatureViewModel, {
            id,
            name: buildMultilingualTextWithSingleItem(
                buildSpatialFeatureText(id),
    ...
    describe(`fetchById`, () => {
        ...
        describe(`when there is a spatial feature with the given ID`, () => {
            it(`should return the expected view`, async () => {
                ...
                const { name } = result as SpatialFeatureViewModel;

                const foundOriginalTextForSpatialFeature = name.items.find(
                    ({ languageCode }) => languageCode === originalLanguageCode
                ).text;

                expect(foundOriginalTextForSpatialFeature).toBe(spatialFeatureText);
            });
        });
    });
});
