...
import { BibliographicSubjectCreatorType } from '../../../../../../../../../libs/data-types/src';
...
import AggregateIdNotAvailableError from '../../../../shared/common-command-errors/AggregateIdNotAvailableError';
        ...
        it('should succeed', async () => {
            await assertCreateCommandSuccess(assertionHelperDependencies, {
                ...
                checkStateOnSuccess: async ({ id }: CreateBookBibliographicReference) => {
                    // It's time to break out this logic and create a shared assertion
                    const idStatus = await idManager.status(id);

                    expect(idStatus).toBe(NotAvailable);

                    const searchResult = await testRepositoryProvider
                        .forResource<BookBibliographicReference>(
                    ...
                    expect((searchResult as BookBibliographicReference).id).toBe(id);
                },
            });
        });
    });
                ...
                await assertCreateCommandError(assertionHelperDependencies, {
                    ...
                    checkError: (error: InternalError) =>
                        assertErrorAsExpected(
                            error,
                            new CommandExecutionError([
                                new AggregateIdNotAvailableError(dummyNewUuid),
                            ])
                        ),
                });
            });
