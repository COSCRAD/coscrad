...
export const buildSpatialFeatureSubtypeFuzzTestCases = (
    geometricFeatureType: GeometricFeatureType
): AggregateFactoryInalidTestCase[] =>
    Object.entries(
        getCoscradDataSchema(getDataCtorFromBibliographicReferenceType(geometricFeatureType))
    ).flatMap(([propertyName, propertySchma]) => {
        const validInstance = getValidBibliographicReferenceInstanceForTest(geometricFeatureType);
