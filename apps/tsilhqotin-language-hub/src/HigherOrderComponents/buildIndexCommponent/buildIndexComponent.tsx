...
const buildStreamlinedViewmodelForTable = <T extends Record<string,unknown>, U extends keyof T>(viewModel: T,keysToKeep: U[]): Omit<T,U> => Object.entries(viewModel)
.reduce(
 (acc: Omit<T,U>,[key,value]: [keyof T,unknown]) =>{
...
const buildIndexComponent = <T extends Record<string,unknown>,>(propertyKeysAndHeadings: PropertyKeyAndHeading<keyof T>[],renderDetailLink: DetailPageLinkRenderer,fetchManyEndpoint: string) => {
    return () => {
    ...
    const rows: GridRowsProp = searchResults.selectedViewModels
        ...
        // @ts-expect-error fix this later
        .map(viewModel => buildStreamlinedViewmodelForTable(viewModel,propertyKeysAndHeadings.map(({propertyKey})=>propertyKey)));
