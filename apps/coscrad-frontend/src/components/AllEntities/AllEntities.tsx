...
export function AllEntities() {
    ...
    if (Object.keys(appState.entitiesAndDescriptions).length === 0) return <div>Loading...</div>;

    return (
        <div>
            <h1>Welcome to AllEntities!</h1>
            <div>
                {Object.entries(appState.entitiesAndDescriptions).map(
                    ([entityType, description]) => (
                        <div key={entityType} data-cy-entity-type={entityType}>
                            <b>{entityType}:</b>&nbsp;{description}
                        </div>
                    )
                )}
            </div>
