...
export const Web3DTestDirect = ({ nodes, connectionsById }: Web3DTestThreeReact): JSX.Element => {
    const onSceneMount = (e: SceneEventArgs) => {
        ...
        let advancedTexture: AdvancedDynamicTexture = AdvancedDynamicTexture.CreateFullscreenUI(
            'ui1',
            true,
            scene
        );
        ...
        for (let i = 0; i < nodes.length; i++) {
            ...
            spheres[i] = BABYLON.MeshBuilder.CreateSphere(`sphere_${i}`, { diameter: 10 }, scene);

            let label: Rectangle = new Rectangle('label for ' + spheres[i].name);
            ...
            // spheres[i].onPointerDownObservable.add(() => {
            //     console.log(spheres[i].name + ' pushed.');
            // });
        }
